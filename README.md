...
- **Framework**: Flutter
- **Language**: Dart
- **State Management**: Riverpod, Indexed Stack Bottom Navigation Bar
...
쿼리의 속도가 느린 탓에 바텀네비게이션 바를 통해 여러 페이지를 이동하는 상황에서 쿼리 충돌이 생기는문제가 있었습니다. 그리고 바텀네비게이션 바의 버튼을 누를때 마다 해당 페이지를 다시 부르는 문제가 존재했습니다. 

#### 해결방안 
- mounted 를 통해 클릭이 여러번 되어도 같은 페이지 내에서는 빌드가 다시 일어나지는 않도록 조절 해 두었습니다. 
- Indexed Stack을 통해 화면들을 재구성하면서 다른 페이지 사이에서 이동할때 한 페이지가 로딩 완료되지않고 dispose 되는것을 방지하였습니다. 
